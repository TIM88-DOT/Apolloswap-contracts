...
import { ApolloswapFactory, ApolloswapPair } from "../../typechain-types";
...
describe("ApolloswapFactory", () => {
  async function fixture() {
    const tmp = await ethers.getContractFactory("ApolloswapFactory");
  ...
  async function createPair(
    factory: ApolloswapFactory,
    tokens: [string, string],
  ) {
    const pairContract = await ethers.getContractFactory("ApolloswapPair");
    ...
    await expect(factory.createPair(tokens[0], tokens[1])).to.be.reverted; // Apolloswap: PAIR_EXISTS
    await expect(factory.createPair(tokens[1], tokens[0])).to.be.reverted; // Apolloswap: PAIR_EXISTS
    ...
    const pair = pairContract.attach(create2Address) as ApolloswapPair;
  ...
  it("setFeeTo", async () => {
    ...
    ).to.be.revertedWith("Apolloswap: FORBIDDEN");

...
import { ApolloswapPair } from "../../typechain-types";

describe("ApolloswapRouter", () => {
  async function v2Fixture() {
    ...
    // deploy V2
    const v2factory = await ethers.getContractFactory("ApolloswapFactory");
    ...
    // deploy routers
    const router = await ethers.getContractFactory("ApolloswapRouter");
    ...
    const pairFactory = await ethers.getContractFactory("ApolloswapPair");
    const pair = (await pairFactory.attach(pairAddress)) as ApolloswapPair;
  ...
  it("quote", async () => {
    ...
    await expect(router.quote(0n, 100n, 200n)).to.be.revertedWith(
      "ApolloswapLibrary: INSUFFICIENT_AMOUNT",
    );
    await expect(router.quote(1n, 0n, 200n)).to.be.revertedWith(
      "ApolloswapLibrary: INSUFFICIENT_LIQUIDITY",
    );
  ...
  it("getAmountOut", async () => {
    ...
    await expect(router.getAmountOut(0n, 100n, 100n)).to.be.revertedWith(
      "ApolloswapLibrary: INSUFFICIENT_INPUT_AMOUNT",
    );
  ...
  it("getAmountIn", async () => {
    ...
    await expect(router.getAmountIn(0n, 100n, 100n)).to.be.revertedWith(
      "ApolloswapLibrary: INSUFFICIENT_OUTPUT_AMOUNT",
    );
  ...
  it("getAmountsOut", async () => {
    ...
    ).to.be.revertedWith("ApolloswapLibrary: INVALID_PATH");
